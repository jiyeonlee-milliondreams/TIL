def my_multi(number_1, number_2):
    return number_1 * number_2 # 함수 수정

result_1 = my_multi(2, 3) # 호출하고 할당하나 할당만 하나 동일하기 때문에 짧게 할당만 하기!!
print(result_1)
# my_multi(2, 3) 결과 : 6
# 함수를 수정하고 호출 결과를 result_1 변수에 할당하여 출력하시오.


def is_negative(number):
    if number <= 0:
        return 'True'
    else:
        return 'False'

result_2 = is_negative(3) 
print(result_2)
# is_negative(3) 결과 : False
# 함수를 수정하고 호출 결과를 result_2 변수에 할당하여 출력하시오.


default = '기본 값'
def default_arg_func(default):
    return default

result_3 = default_arg_func(default) # 함수를 호출하는 경우를 제외하고는 인자를 빈칸으로 둘 수 없음. 
result_4 = default_arg_func('다른 값')
print(result_3)
print(result_4)

# 오류 일지: print()를 쓰면 return이 None으로 나오게 됨. 
# def is_negative(number):
#     if number <= 0:
#         print('True')
#     else:
#         print('False')